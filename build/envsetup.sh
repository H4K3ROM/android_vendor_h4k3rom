function __print_h4k3rom_functions_help() {
cat <<EOF
Additional H4K3ROM functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- h4k3romgerrit:   A Git wrapper that fetches/pushes patch from/to H4K3ROM Gerrit Review.
- h4k3romrebase:   Rebase a Gerrit change and push it again.
- h4k3romremote:   Add git remote for H4K3ROM Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for H4K3ROM Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the H4K3ROM model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch h4k3rom_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/h4k3rom_*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        adb wait-for-online
        echo "Found device"
        if (adb shell getprop ro.h4k3rom.device | grep -q "$H4K3ROM_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $H4K3ROM_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function h4k3romremote()
{
    ...
    git remote rm h4k3rom 2> /dev/null
    ...
    local H4K3ROM_USER=$(git config --get review.gerrit.h4k3rom-rom.username)
    if [ -z "$H4K3ROM_USER" ]
    then
        git remote add h4k3rom ssh://gerrit.h4k3rom-rom:29418/$PFX$PROJECT
    else
        git remote add h4k3rom ssh://$H4K3ROM_USER@gerrit.h4k3rom-rom:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/H4K3ROM/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-online
    if (adb shell getprop ro.h4k3rom.device | grep -q "$H4K3ROM_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        h4k3romremote
        git push h4k3rom HEAD:refs/heads/'$1'
    fi
    '
}

function h4k3romgerrit() {
    ...
    local user=`git config --get review.gerrit.h4k3rom-rom.com.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "h4k3romgerrit" ]; then
...
function h4k3romrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "H4K3ROM Gerrit Rebase Usage: "
        echo "      h4k3romrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://gerrit.h4k3rom-rom.com/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.h4k3rom.device | grep -q "$H4K3ROM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/h4k3rom/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $H4K3ROM_FIXUP_COMMON_OUT ]; then
